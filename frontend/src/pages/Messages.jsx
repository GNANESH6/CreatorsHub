...
const Messages = () => {
  ...
  useEffect(() => {
    ...
    const initChat = async () => {
      try {
        ...
        const resConn = await axios.post('http://localhost:5002/api/chat/create', { userId: id }, {
          headers: { Authorization: `Bearer ${token}` }
        });
        setConversation(resConn.data);

        const resMsgs = await axios.get(`http://localhost:5002/api/chat/messages/${resConn.data._id}?page=1&limit=50`, {
          headers: { Authorization: `Bearer ${token}` }
        });
        ...
        if (!socketRef.current) {
          socketRef.current = io('http://localhost:5002', { auth: { token } });
        }
  ...
  const setupPushNotifications = async (token) => {
    try {
      ...
      if (!subscription) {
        const res = await axios.get('http://localhost:5002/api/users/push/vapidKey', {
          headers: { Authorization: `Bearer ${token}` }
        });
      ...
      await axios.post('http://localhost:5002/api/users/push/subscribe', { subscription }, {
        headers: { Authorization: `Bearer ${token}` }
      });
    } catch (err) {
  ...
  const handleScroll = async (e) => {
    if (e.target.scrollTop === 0 && hasMore && !isLoadingMore && conversation) {
      ...
      try {
        ...
        const token = sessionStorage.getItem('token');
        const res = await axios.get(`http://localhost:5002/api/chat/messages/${conversation._id}?page=${nextPage}&limit=50`, {
          headers: { Authorization: `Bearer ${token}` }
        });
  ...
  const handleSend = async (e) => {
    ...
    try {
      const token = sessionStorage.getItem('token');
      const res = await axios.post('http://localhost:5002/api/chat/send', {
  ...
  const handleClearChat = async () => {
    ...
    try {
      const token = sessionStorage.getItem('token');
      await axios.post('http://localhost:5002/api/chat/clear', { conversationId: conversation._id }, {
         headers: { Authorization: `Bearer ${token}` }
      });
  ...
  const handleDeleteMessage = async (type) => {
    ...
    try {
      const token = sessionStorage.getItem('token');
      await axios.delete(`http://localhost:5002/api/chat/messages/${selectedMessage._id}`, {
        headers: { Authorization: `Bearer ${token}` },
