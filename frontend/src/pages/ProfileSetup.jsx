...
const ProfileSetup = () => {
  ...
  useEffect(() => {
    const fetchUser = async () => {
      ...
      try {
        ...
        const resPort = await axios.get(`http://localhost:5002/api/portfolio/${d._id}`, {
           headers: { Authorization: `Bearer ${token}` }
        });
  ...
  const handleAddPortfolioItem = async () => {
    ...
    try {
      const res = await axios.post('http://localhost:5002/api/portfolio', formData, {
        headers: { 
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // 1. Update Profile
      await axios.put('http://localhost:5002/api/users/profile', {
  ...
  const handleDeleteAccount = async () => {
    if (window.confirm("Are you sure you want to delete your account?")) {
      ...
      try {
        const token = sessionStorage.getItem('token');
        await axios.delete('http://localhost:5002/api/auth/delete', { headers: { Authorization: `Bearer ${token}` } });
