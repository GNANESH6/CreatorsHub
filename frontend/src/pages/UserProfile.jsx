...
const UserProfile = () => {
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        if (foundUser) {
          ...
          const meRes = await axios.get(`${import.meta.env.VITE_API_URL.replace(/\/+$/, '')}/api/auth/me`, {
            headers: { Authorization: `Bearer ${token}` }
          });
        ...
        const resRev = await axios.get(`${import.meta.env.VITE_API_URL.replace(/\/+$/, '')}/api/reviews/${id}`, {
          headers: { Authorization: `Bearer ${token}` }
        });
        setReviews(resRev.data);

        const resPort = await axios.get(`${import.meta.env.VITE_API_URL.replace(/\/+$/, '')}/api/portfolio/${id}`, {
          headers: { Authorization: `Bearer ${token}` }
        });
        setPortfolio(resPort.data);

        const [resConn, resInc, resOut] = await Promise.all([
          axios.get(`${import.meta.env.VITE_API_URL.replace(/\/+$/, '')}/api/collaboration/connections`, { headers: { Authorization: `Bearer ${token}` } }),
          axios.get(`${import.meta.env.VITE_API_URL.replace(/\/+$/, '')}/api/collaboration/incoming`, { headers: { Authorization: `Bearer ${token}` } }),
          axios.get(`${import.meta.env.VITE_API_URL.replace(/\/+$/, '')}/api/collaboration/outgoing`, { headers: { Authorization: `Bearer ${token}` } })
        ]);
  ...
  const handleRespond = async (status) => {
    try {
      const token = sessionStorage.getItem('token');
      await axios.put(`${import.meta.env.VITE_API_URL.replace(/\/+$/, '')}/api/collaboration/${pendingCollabId}`, { status }, {
        headers: { Authorization: `Bearer ${token}` }
      });
  ...
  const handleRequestCollab = async () => {
    try {
      const token = sessionStorage.getItem('token');
      await axios.post(`${import.meta.env.VITE_API_URL.replace(/\/+$/, '')}/api/collaboration/send`, { receiverId: id }, {
        headers: { Authorization: `Bearer ${token}` }
      });
      setCollabStatus('pending');
  ...
  const handleSubmitReview = async (e) => {
    ...
    try {
      const token = sessionStorage.getItem('token');
      const res = await axios.post(`${import.meta.env.VITE_API_URL.replace(/\/+$/, '')}/api/reviews`, {
        revieweeId: id,
        rating,
        comment
      }, {
